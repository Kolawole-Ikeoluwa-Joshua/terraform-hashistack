...
mkdir -p /usr/local/bin/vtl-linuxapp
cd /usr/local/bin/vtl-linuxapp
python3 -m venv venv
...
# Gunicorn config
cat <<EOF > '/usr/local/bin/vtl-linuxapp/wsgi.py'
from app import app
...
cat <<EOF > '/usr/lib/systemd/system/vtl-linux-python-app.service'
[Unit]
Description=vtl-linux Python Example App
After=consul.service

[Service]
WorkingDirectory=/usr/local/bin/vtl-linuxapp
Environment="PATH=/usr/local/bin/vtl-linuxapp/venv/bin"
ExecStart=/usr/local/bin/vtl-linuxapp/venv/bin/gunicorn --workers 3 --bind unix:myproject.sock -m 007 wsgi:app
Restart=always
...
cat <<EOF > '/etc/consul.d/python-service.json'
{
  "service": {
    ...
    "address": "",
    "meta": {
      "meta": "a vtl-linux Python web application example"
    },
    "port": 5000,
...
cat <<EOF > '/etc/nginx/sites-available/vtl-linux-python'
server {
    listen 80;
...
EOF
ln -s /etc/nginx/sites-available/vtl-linux-python /etc/nginx/sites-enabled
...
echo "Enabling and starting the vtl-linux-python-app service..."
systemctl enable vtl-linux-python-app
systemctl start vtl-linux-python-app
